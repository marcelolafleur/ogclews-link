...
\begin{frame}[plain]
  ...
  {\small\color{mute}Matthew LaFleur · August 2026 ·
...
\dgmframe{The structural seam}{architecture}{%
  ...
  separate and pass a small set of auditable signals.}
...
% 5 ----------------------------------------------------------------------------
\begin{frame}{The vocabulary: signal, wedge, channel}
  \begin{description}
    \item[{\color{clews}Signal}] a quantity read from a \emph{solved} \CLEWS{}
      scenario: the marginal cost of a fuel, the capital a build-out requires,
      an emissions path, the cost implied by an emissions cap.
    \item[{\color{ogc}Wedge}] the \OGCore{} parameter that expresses it: a
      consumption-price wedge $\tauc$, an industry productivity $Z_m$, the
      public-investment share $\alphaI$, mortality $\rho(s,t)$, effective labour
      $e(t,s,j)$.
    \item[Channel] a named, guard-railed map from a signal to a
      \emph{deliberately partitioned set of wedges that never overlap} --- one
      wedge in the simplest channels, several where a signal splits by use
      (energy) or by demographic object (health).
  \end{description}
  \medskip
  ...
  \srcnote{paper/sections/04-framework.tex, signals and wedges (partitioned-wedge
  definition after the review round of 2026-08-04).}
\end{frame}
...
% 7 ----------------------------------------------------------------------------
\begin{frame}{Electricity enters through firms' costs and household budgets}
  \centering
  \resizebox{0.94\linewidth}{!}{%
  \chainrow{electricity price,\\reform/baseline ratio $g_t$}{%
    intermediate share: cost-push on $Z_m$\\
    final share: wedge $1+\tau^{c,r}_{i_e}=g_t(1+\tau^{c,b}_{i_e})$}{%
    cost of production $\uparrow$\\cost of living $\uparrow$, regressive}}
  \medskip
  \begin{itemize}\small\setlength\itemsep{3pt}
    \item the price splits by \emph{use}: roughly three quarters of Philippine
          electricity is an industry input (an input--output-calibrated cost-push
          on industry productivities), one quarter household consumption
          (a consumption-price wedge); self-use netted so nothing enters twice
    \item neither side alone is the channel: the wedge is close to output-neutral,
          the cost-push contractionary --- only the composite carries the price
          into both margins
    \item with a subsistence floor, poorer households spend a larger energy share
          $\Rightarrow$ incidence is regressive even under full revenue recycling
  \end{itemize}
  \smallskip
  {\color{sub}\small The structural destination --- energy as a priced production
  input --- is specified but out of scope.}
  \srcnote{paper/sections/04-framework.tex, the energy channel; SAM shares in
  paper/sections/06-calibration.tex.}
\end{frame}
...
\begin{frame}{The eight channels}
  ...
  \end{tabularx}
  \par\medskip
  {\color{sub}\small four carry the energy system into the economy · two are policy
  levers · two return the economy's answers}
...
% 11 ---------------------------------------------------------------------------
\begin{frame}{The model's own first-order condition settles the capital question}
  \footnotesize
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      Under Cobb--Douglas production, energy-industry capital demand satisfies
      \[
        K_E=\frac{\gamma_E\,p_E Y_E}{\rho_E},
        \qquad
        \hat K_E=\hat\gamma_E\,\hat p_E\,\hat Y_E
      \]
      (the user cost $\rho_E$ and taxes cancel in the reform/baseline ratio).
      \smallskip

      Raising the capital \emph{share} does not pull capital in: the higher
      exponent cuts unit cost, the electricity price collapses, and revenue ---
      hence capital --- falls, despite higher output. The instrument that does
      draw capital in is the tax credit, which enters $\rho_E$ where
      $\gamma_E$ is absent.
    \end{column}
    \begin{column}{0.44\textwidth}
      ...
      \begin{tabular}{@{}lr@{}}
        \toprule
        Earlier M=8 steady-state solve & Change \\
        \midrule
      ...
      {\color{sub}\scriptsize Evidence status: recomputed from the archived solved
      steady states (2026-08-04); a re-run under the re-blessed baseline is
      queued.}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.44\textwidth}
      {\footnotesize\begin{itemize}\setlength\itemsep{3pt}
        \item \CLEWS{} supplies the PM$_{2.5}$ emissions path; the Global Burden
              of Disease supplies the burden and its age profiles
        \item a calibrated dose-response scales the $2.7\%$ emissions cut to
              about $95$ fewer deaths per year --- not the $1{,}165$ of
              one-for-one scaling
        ...
        \item \textbf{morbidity} is working-age $\to$ effective labour rises ---
              the near-term macro gain ($+0.19\%$)
      \end{itemize}}
      \smallskip
      {\color{sub}\scriptsize The morbidity productivity elasticity remains an
      application assumption.}
    \end{column}
    \begin{column}{0.53\textwidth}
      \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{health_age_profiles.png}
    \end{column}
  \end{columns}
...
% 16 ---------------------------------------------------------------------------
\begin{frame}{The Philippine application: what is active in the coupled run}
  \centering
