...
side; industrial carbon is priced only in \CLEWS{}. In the present one-pass validation
the \CLEWS{}-side path is \emph{emitted} as a ready input; it shapes the build mix only
when the energy model is re-solved on it. Revenue is recyclable through
...
capital---leaving risk premia, tax treatment, and the real/nominal convention as
explicit harmonisation choices rather than resolved economics. The effect on the macro
