...
Philippines' \emph{real} \CLEWS{} value---the ``auto'' source: a cost-of-electricity
index where a curated workbook exists, otherwise the \OSeMOSYS{} commodity-balance marginal cost
on the raw run---not an illustrative shock; it is near-flat for the Philippine scenario,
so the channel's effect is small. Incidence is the channel's distinctive output: with
