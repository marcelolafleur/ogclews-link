...
\midrule
\chan{energy\_price} & c$\to$o & electricity price (marginal cost / index) & cost-push + consumption wedge & output $\downarrow$; regressive incidence \\
...
electricity price into the economy through both of electricity's uses---a cost-push on
...
electricity's self-use netted out so the two do not overlap. The price is the
Philippines' \emph{real} \CLEWS{} value---the ``auto'' source: a curated
cost-of-electricity index where a workbook exists, otherwise the levelized cost of
electricity (LCOE) reconstructed from the run's costs and generation (the commodity-balance
shadow price is degenerate here, so it is not used)---not an illustrative shock. For the
Philippine reform the LCOE rises about $5\%$ on average, so the channel has a small but
non-negligible effect. Incidence is the channel's distinctive output: with
